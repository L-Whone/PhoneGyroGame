...
public class GPSFind : MonoBehaviour
{
    ...
    [SerializeField] private float targetLongitude = -123.10744680000001f;
    ...
    private readonly Color coloCold = new Color(0f, 0f, 1f);
    private readonly Color colorLukewarm = new Color(0.5f, 1f, 0.5f);
    ...
    {
        // F1 is decimal place how many decimals
        distanceText.text = distance >= 1000f ? $"{distance / 1000f:F1} km away" : $"{distance:F0} m away";
    ...
    private void UpdateColorIndicator(float distance)
    {
        ...
        if (distance > coldRadius) target = coloCold;
