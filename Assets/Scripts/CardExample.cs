...
public class CardExample : MonoBehaviour
{
    ...
    [SerializeField] private float maxSwipTime = 0.4f;
    ...
    private void NavigateTo(int index)
    {
        ...
        StartCoroutine(CoroutineSlideCards(currentIndex, index));
        currentIndex = index;
    }

    private IEnumerator CoroutineSlideCards(int from, int to)
    {
        isAnimating = true;
    ...
    {
        // TODO here
        // Atan 2 takes x,y and gives the angle of that point in space from the horizontal
        // we are getting the angle between two touch points to rotate the image

        float currentDistance = Vector2.Distance(t0.screenPosition, t1.screenPosition);

        // gives the angle of the two points
